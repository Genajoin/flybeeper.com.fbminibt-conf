...
![FlyBeeper TAS — трубка Пито и хвостовое оперение удерживают прибор по потоку под подвеской](/manual-media/fbtas/03-mount.jpg)
...
Итоговое устройство имеет встроенную солнечную панель и не предполагает иного способа зарядки. Перед использованием рекомендуется положить на солнце на открытом воздухе на 1 час.

Прототип устройства имеет разъем `USB Type C` для зарядки. Он поддерживает только простые зарядные устройства, на выходе которых всегда присутствует 5 В, например, на `USB` выходах персонального компьютера. Умные зарядные устройства и `PD` не поддерживаются.

### Настройка

Для работы необходимо устройство, имеющее Bluetooth модуль, например, смартфон или ноутбук или ПК с Bluetooth модулем. Нажмите `Connect` и выберите из списка `FBTAS`. Держите устройства как можно ближе друг к другу. Измените любой параметр и нажмите `Apply`. Посмотреть все данные, отправляемые устройством в реальном времени, можно на вкладке `Cockpit`.

`Pitot K factor (o.e.)` - калибровочный коэффициент, учитывающий конструкционную потерю давления в трубке Пито и тракте к датчику дифференциального давления. При увеличении коэффицента пропорционально увеличиваются показания IAS и TAS. Для калибровки имеется специальная утилита - [TAS calibration](https://flybeeper.com/utils/tas-calibration/).
